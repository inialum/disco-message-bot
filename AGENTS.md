...
- Worker bindings types are generated with `pnpm typegen` (`wrangler types --env-interface CloudflareBindings --strict-vars false`) into `worker-configuration.d.ts`. Re-run after Wrangler config (`wrangler.jsonc`) changes.
